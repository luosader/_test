<?php
/**
 * 注意：所有图片的截取，缩放都要用到该文件
 * 注意：文件最后修改时间2015年9月29日..作者：明哥先生
 */
header('Content-type:text/html;charset=utf-8'); //编码
error_reporting(E_ALL & ~E_NOTICE & ~E_WARNING); //关闭错误提示
require ('shearphoto.config.php'); //加载配置文件
...
// HTML5截取时
elseif ( isset($_POST['ShearPhotoIW']) && isset($_POST['ShearPhotoIH']) && isset($_POST['ShearPhotoFW']) && isset($_POST['ShearPhotoFH']) && isset($_POST['ShearPhotoP']) && is_numeric($JSconfig['IW'] = trim($_POST['ShearPhotoIW'])) && is_numeric($JSconfig['IH'] = trim($_POST['ShearPhotoIH'])) && is_numeric($JSconfig['FW'] = trim($_POST['ShearPhotoFW'])) && is_numeric($JSconfig['FH'] = trim($_POST['ShearPhotoFH'])) && is_numeric($JSconfig['P'] = trim($_POST['ShearPhotoP'])) ) {
	$Shear = new ShearPhoto;
	//类实例开始
