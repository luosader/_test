<?php
/**
 * 注意：所有的shearphoto设置都几乎在这里.请细心品味
 * 注意：文件最后修改时间2015年9月日..作者：明哥先生
 */
define('IOURL', dirname(dirname(__FILE__))); //锁定相对目录
define('DS', DIRECTORY_SEPARATOR); //DIRECTORY_SEPARATOR 是斜杠符，为兼容 WINDOW 和 LINUX
define('ShearURL', IOURL . DS);
...
$ShearPhoto['config'] = array(
    ...
    // 自定义设置生成截图的张数，大小，在这设，看好下面！
    'width' => array(
        // array(0, true, 'name0'), //此时的0 代表以用户取当时截取框的所截的大小为宽
        // array(-1, true, 'name1'), //此时的-1 代表以原图为基准，获得截图

        array(150, true, 'big'), //@参数1：要生成的宽（高度不用设，系统会按比例做事）； @参数2：是否为该图加水印，water参数要有水印地址才有效true或false； @参数3：图片后面添加字符串（用以区分其他截图名称),填写字符串，不要含中文，不然能又鸡巴痛了 ，不定义的话默认为“0”

        array(100, true, 'middle'),
    ...
    'water_scope' => 100, //图片小于多少不添加水印！没填水印地址，这里不起任何作用

    'temp' => ShearURL . 'file' . DS . 'temp', //等待截图的大图文件。就是上传图片的临时目录，截图后，图片会被删除,非HTML5切图就会用到它

    'tempSaveTime' => 600, //临时图片（也就是temp内的图片）保存时间，需要永久保存请设为0。单位秒

    'saveURL' => ShearURL . 'file' . DS . 'shearphoto_file' . DS, //截好后的图片。储存的目录位置，后面不要加斜杠，系统会自动给补上！不要使用中文
