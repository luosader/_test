...
window.ShearPhoto.MINGGE(function() {
	...
	Shear.config({
		...
		preview: [150], // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组) 数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图，shearphoto消耗资源越多，性能越差！ 官方不建意你开启这个功能，尽可能请设为preview:false
		...
		proportional: [
			1 / 1, // 截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误
	...
	// 保存截图：按下截图事件，提交到后端的shearphoto.php接收
