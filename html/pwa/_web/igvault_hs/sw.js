...
const CASH_LIST=['js','css','jpg','png','jpeg','gif','svg','ttf','woff','eot','ico','webmanifest'];//js
...
//     return function(){
//         if(logDebug){oriLogFunc.apply(this,arguments);}
//     };
// })(console.log);
...
self.addEventListener('fetch',function(e){
    ...
    // 缓存页面
    if(ignoreReg.test(e.request.url)||/sw\.js/.test(e.request.url)||(!/[/-]js/.test(e.request.url)&&!reg.test(e.request.url))){return;}
    ...
    e.respondWith(caches.match(e.request).then(function(response){
        ...
        return response||fetch(e.request).then(function(response){
            ...
            return caches.open(CACHE).then(function(cache){
                cache.delete(e.request,{ignoreSearch:true});
...
// SW 在安装完成后会被激活，所以我们也可监听 activate 事件。
...
self.addEventListener('notificationclick',function(e){
    ...
    if(data.hasOwnProperty(action)){url=data[action];}
    data.action=action;
