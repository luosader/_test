...
var $modules = {
	...
	"1545034775000" : {
		...
		"context" : "<div class=\"search-con clearfix\">\n  " +
				...
				"<div class=\"search-rightBtn search-click\" >\n      搜索\n    <\/div>\n <div class=\"helpBtn\"><a href=\"/website/wenshu/181109AWZA70BFK4/index.html\" target=\"_blank\"><img src=\"./images/list-help.png\"/><\/a><\/div>\n <\/div>\n  " +
...
$(function() {
	...
	if(nickName!=""&&nickName!=null&&typeof(nickName)!="undefined"){
		$("#loginLi").attr('href','http://wenshu.court.gov.cn/website/wenshu/181029BPRY8AYR1P/index.html?open=login');
	...
	$("#_view_1545034775000").on("keyup", ".searchKey.search-inp", function(event) {
	...
	$("#_view_1545034775000").on("click", ".search-click", function(event) {
		var $m = $('#_view_1545034775000');
		var $mid = "1545034775000";
	...
	$("#_view_1545034775000").on("click", "#searchBtn", function(event) {
		var $m = $('#_view_1545034775000');
		var $mid = "1545034775000";
		// 获取urlParam
		var urlParam = getUrlParam1545034775000();
	...
	$("#_view_1545035259000").on("click", "#save-Btn", function(event) {
		var $m = $('#_view_1545035259000');
		var $mid = "1545035259000";
		// 未登录跳转到登录页start
		var userInfo = $.WebSite.getUserInfo();
		if (userInfo == null || userInfo.loginId == "anonymousUser") {
			if(navigator.userAgent.indexOf("MSIE")>-1){
                ...
                referLink.href= $website["domain"] + $website["home"] + "?"+ window.location.href;
...
function initHead1545034746000(d) {
	...
	function showDate() {
		...
		weekDay[5] = "星期五";
		weekDay[6] = "星期六";
		$("#showDate").text(year + "年" + month + "月" + day + "日" + " "+ weekDay[date.getDay()]);
	}
	showDate();
...
function initYjjy1545034768000(d) {
	...
	// 意见建议提交
	$m.on("click", "span#submitBtn", function() {
				...
				$.WebSite.alert({
					...
					rollback : function() {
						...
						if ($globalNet == 'outer1') {
							...
							$_gsp.push(["T", "GWD-002808", "addOrder", orderid, 1 ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, location.pathname, location.pathname, 1, 1, "Page" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, "意见建议", "意见建议", 1, 1, "FormName" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, suggestType, suggestType, 1, 1, "建议类型" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, suggestContent, suggestContent, 1, 1, "建议内容" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, suggestSource, suggestSource, 1, 1, "案件来源" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, court, court, 1, 1, "所属法院" ]);
							$_gsp.push(["T", "GWD-002808", "addProduct", orderid, remark, remark, 1, 1, "备注" ]);
							// 更多addProduct
							$_gsp.push(["T", "GWD-002808", "trackECom" ]);
						}
...
function getUrlParam1545034775000(d) {
	...
	if ($.trim(flyj).length > 0 && $.trim(flyj) != $(".large_input input").attr("placeholder")) {
		urlParam += "&flyj=" + flyj;
	}
	return urlParam;
};
	...
	if (d && d.length > 0) {
		$.each(d, function(index, obj) {
			...
			// 3、设置法院层级、案件类型、文书类型、公开类型、案例等级
					...
					|| code == 's6' || code == 's43'
					|| code == 's44') {
				var node = $m.find("#" + code).parent().next(".list-box").find("li[data-val='" + value + "']");
				if (node) {
					node.addClass("on");
					var text = node.text();
					...
					if (code == 's8') {// 如果字段是案件类型，重新设置审判程序下拉框数据
						if ($m.find("#spcxTreeDiv").attr("data-val") && $m.find("#spcxTreeDiv").attr("data-val") != null && $m.find("#spcxTreeDiv").attr("data-val") != '') {
							var spcxdData = JSON.parse($m.find("#spcxTreeDiv").attr("data-val"));
							var spcxArray = spcxdData[value];
							$m.find('.list-input .jstree.spcx').data('jstree', false).empty();
			...
			// 4、审判程序
				...
				if ($m.find("#spcxTreeDiv").attr("data-val") 
					...
					if (text == null || text == "") {
						...
						$m.find('.spcx').find('span').html(text);
					}
					$m.find('.spcx').find('span').attr("data-val", value);
					$m.find('.spcx').find('span').attr("data-level", "1");
				}
				...
				if ($m.find("#spcxTreeDiv").attr("data-val")
					...
					$m.find('.spcx').find('span').attr("data-val", value);
					$m.find('.spcx').find('span').attr("data-level", "2");
				}
			}
			// 3、其他字段设值
			else {
...
function cleanFormVal1545034775000(d) {
	...
	jsonArray.push({id : "flyj", value : ""});
	jsonArray.push({id : "cprqStart", value : ""});
	jsonArray.push({id : "cprqEnd", value : ""});
	...
	for (var i = 0; i < 60; i++) {
		...
		var id = "s" + i;
		var value = "";
		jsonArray.push({id : id, value : value });
	}
...
function initSearch1545034775000(d) {
	...
	// 全文下拉选项选择事件
	$(".selectCon span").parent().next(".list-box").on("click", "li", function() {
	...
	// 文书类型、法院层级、案件类型
	$(".selectCon_other span").parent().next(".list-box").on("click", "li", function() {
		$(this).addClass('on').siblings('li').removeClass('on');
		var txt = $(this).html();
		var txtCode = $(this).attr("data-val");
	...
	$m.on('click', '.list-input .jstree.spcx a', function() {
		...
		$m.find('.spcx').find('span').attr("data-val", txtCode.replace("_anchor", ""));
		var dataLevel = $(this).parent().attr("aria-level");
...
//		});
	...
	$m.find("#selectCon_other_ajlx span").parent().next(".list-box").find("li").click(function() {
		$(this).addClass('on').siblings('li').removeClass('on');
		var txt = $(this).html();
		...
		if ($m.find("#spcxTreeDiv").attr("data-val") 
			&& $m.find("#spcxTreeDiv").attr("data-val") != null 
			&& $m.find("#spcxTreeDiv").attr("data-val") != '') {
			var spcxJsonData = JSON.parse($m.find("#spcxTreeDiv").attr("data-val"));
...
function addParams1545035259000(d) {
	...
	for ( var i in d) {
		...
		// 转换关键字为多值（使用逗号分隔）
			...
			$.each(s45Arr, function(i, item) {
				if ($m.find(".LT_Filter_right p[data-key='s45'][data-value='" + item + "']").length == 0) {
	...
	function appendSearchItem(key, value, text) {
		if ($m.find(".LT_Filter_right p[data-key='" + key + "'][data-value='"+ value + "']").length != 0) {
			searchFlag = false;
			return;
		}
		...
		$m.find('.LT_Filter_right').append('<p data-key="' + key + '" data-value="' + value + '">' + text + ' <i class="fa fa-close"></i></p>');
	}
};
...
function initTree1545094675000(d) {
	...
	function loadData(obj, callback) {
		if (obj.parents.length !== 0) {
			...
			var nodeParams = typeof d.getNodeParams === "function" ? d.getNodeParams(obj, groupField) : {};
			// 设置查询参数(页面需要传递查询条件模块的id：searchMid)
			var searchData = eval("addParams" + searchMid + "("+ JSON.stringify(d.postData || {}) + ")");
...
function initEvents1545184311000(d) {
	...
	var searchMid = d.searchMid;
	// 排序
	$m.off("click", "div.tool_PX").on("click", "div.tool_PX", function(e) {
		if ($(this).hasClass("tool_On")) {
			$(this).removeClass("tool_On").addClass("tool_OnUp");
		} else {
			$(this).removeClass("tool_OnUp").addClass("tool_On");
		}
		$(this).siblings(".tool_PX").removeClass("tool_On").removeClass("tool_OnUp");
		// 刷新列表
	...
	// 全选
	$m.off("change", "input#AllSelect").on("change", "input#AllSelect", function(e) {
		if (this.checked) {
			$(this).parent().css({
	...
	// 单选
	$m.off("change", "input.ListSelect").on("change", "input.ListSelect", function(e) {
		if (this.checked) {
			$(this).parent().css({
				'background' : 'url(../images/list/AllSelect_on.png) left center no-repeat'
			});
	...
	// 批量收藏
	$m.off("click", "a.AllKeep").on("click", "a.AllKeep", function(e) {
		var checkedNum = $('.ListSelect:checked').length;
		if (checkedNum == 0) {
			$('body').append('<div class="PopBox-Toast clearfix"><div class="Pop_left fl"><i class="fa fa-frown-o"></i></div><div class="Pop_right fr">请选择收藏内容</div></div>');
		...
		$('.ListSelect').each(function() {
			if (this.checked) {
				...
				var caseInfoItem = {};
				var thisLM_list = $(this).parents('.LM_list');
				packageName = thisLM_list.find(".caseName").text();
				caseInfoItem["caseName"] = thisLM_list.find(".caseName").text();
				caseInfoItem["slfyName"] = thisLM_list.find(".slfyName").text();
				caseInfoItem["ah"] = thisLM_list.find(".ah").text();
				caseInfoItem["cprq"] = thisLM_list.find(".cprq").text();
				caseInfoItem["id"] = $(this).attr("data-value");
	...
	// 批量下载
	$m.off("click", "a.AllDownload").on("click", "a.AllDownload", function(e) {
		var checkedNum = $('.ListSelect:checked').length;
		if (checkedNum == 0) {
			$('body').append('<div class="PopBox-Toast clearfix"><div class="Pop_left fl"><i class="fa fa-frown-o"></i></div><div class="Pop_right fr">请选择下载内容</div></div>');
			$('.PopBox-Toast').fadeIn().delay(1500).fadeOut();
			setTimeout(function() {
				$('.PopBox-Toast').remove();
			}, 1500);
	...
	// 收藏
	$m.off("click", "a.a_sc").on("click", "a.a_sc", function(e) {
		// 未登录跳转到登录页start
		var userInfo = $.WebSite.getUserInfo();
		if (userInfo == null || userInfo.loginId == "anonymousUser") {
			if(navigator.userAgent.indexOf("MSIE")>-1){
		...
		var caseInfo = {};
		$(this).parents(".LM_list").each(function() {
		...
		var packageName = $(this).parent().parent().parent().find(".caseName").text();
		$.WebSite.saveData({
			allowTip : false,
			param : {
				id : thisId,
	...
	// 下载
	$m.off("click", "a.a_xz").on("click", "a.a_xz", function(e) {
		var thisId = $(this).attr("data-value");
		window.location.href = "/down/oneDoc?docId="+ thisId;
		return false;
	});
	...
	// 每页条数
	$m.off("change", ".pageSizeSelect").on("change", ".pageSizeSelect", function(e) {
		$.WebSite.invoke($mid, "loadData", {
			searchMid : searchMid,
			pageNum : 1
		});
	...
	// 关联文书排版
	$('.list_Association .guanLian').each(function() {
	...
	$('.list_Association').each(function() {
	...
	$.WebSite.getModule($mid).find(".list_title.clearfix h4 a").each(function(i, item) {
		$(this).html($.WebSite.keywordHighlight($(this).text() || '', highLightArr || ''));
	});
	$.WebSite.getModule($mid).find(".list_reason p").each(function(i, item) {
		$(this).html($.WebSite.keywordHighlight($(this).text() || '', highLightArr || ''));
	});
};
