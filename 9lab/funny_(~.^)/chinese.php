...
/*$n = new Bihua();
var_dump($n->countBihua('谢'));
$n->stat('谢春花');
var_dump($n->result);
$arr = ['谢', '春', '花'];
var_dump($n->sortBihua($arr, 'desc'));*/
...
class Bihua
{
     ...
     * 查询汉字笔画
     */
    public function countBihua($str)
    {
        // 数字
        $index = strstr($this->dataNumber, $str);
        if ($index !== false) {$this->numberCnt++;return 0;}
        // 英文
        $index = strstr($this->dataLetter, $str);
        if ($index !== false) {$this->letterCnt++;return 0;}

        // 汉字
        for ($i = 0; $i < 36; $i++) {
            for ($j = 0; $j < 5; $j++) {
                if (!isset($this->data[$i][$j])) {continue;}
    ...
    public function stat($str)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $char_num = $this->countBihua($char); //获得笔画
            if ($char_num != 0) {
                $num += $char_num;
        ...
        $this->result = array(
            'numberCnt' => $this->numberCnt,
            'letterCnt' => $this->letterCnt,
            'elseCnt'   => $this->elseCnt,
     ...
     * 按笔画排序
     * @param  [type] $array 汉字
     * @param  string $order 按笔画数大小 asc顺序; desc倒序;
     * @return string        返回排序后的字符串
     */
    public function sortBihua($array, $order = 'asc')
    {
        ...
        foreach ($array as $str) {
            $sortArray[$str] = $this->countBihua($str);
        }
        if ($order == 'asc') {
            asort($sortArray);
        } else {
