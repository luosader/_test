/**
 * 说明 console.log(this)
 * 1、fileTypeExts 限定格式在选文件对话框中不起作用 => 展示了所有类型文件
 *   <<已改良>>
 *     <input type="file" name="file" accept="image/*,text/plain,.xls">
 *     <input type="file" name="pic" accept=".jpg,.jpeg,.png,.gif,.bmp">
 * 2、不能自定义文件命名
 * 3、断点续传
...
(function($){
    $.fn.Huploadify = function(opts){
        ...
        var itemTemp = '<span></span>';
        var defaults = {
            fileTypeExts:'*.*',//允许上传的文件类型，格式'*.jpg;*.doc'
            uploader:'/Img/upload',//文件提交的地址; ''
            auto:true,//false; 是否开启自动上传
            method:'post',//发送请求的方式，get或post
            multi:false,//true; 是否允许选择多个文件
            formData:null,//发送给服务端的参数，格式：{key1:value1,key2:value2}
            fileObjName:'Filedata',//在后端接受文件的参数名称，如PHP中的$_FILES['file']
            fileSizeLimit:2048,//允许上传的文件大小，单位KB
            showUploadedPercent:false,//true; 是否实时显示上传的百分比，如20%;
            showUploadedSize:false,//是否实时显示已上传的文件大小，如1M/2M
            buttonText:'SELECT FILES',//'选择文件'; 上传按钮上的文字
                        ...
                        xhr.onreadystatechange = function(e) {
                            if(xhr.readyState == 4){
                                if(xhr.status == 200){
                                    ...
                                    option.onUploadSuccess && option.onUploadSuccess(file, xhr.responseText, true);
                                    //在指定的间隔时间后删掉进度条
                                    setTimeout(function(){
                                        _this.find('#fileupload_'+instanceNumber+'_'+file.index).fadeOut();
